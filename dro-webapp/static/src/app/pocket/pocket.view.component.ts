...
export class PocketViewComponent {
  ...
  getPocketDrillTransform() {
    ...
    let xMin = Math.min(this.pocket.p1.x, this.pocket.p2.x);
    let xMax = Math.max(this.pocket.p1.x, this.pocket.p2.x);
    let yMin = Math.min(this.pocket.p1.y, this.pocket.p2.y);
    let yMax = Math.max(this.pocket.p1.y, this.pocket.p2.y);
    let xLocation;
    let yLocation;
  ...
  getPocketDrillTransformZ() {
    ...
    let zMax = Math.max(this.pocket.p1.z, this.pocket.p2.z);
    let zLocation;
  ...
  getPointDistance(c1: number, c2: number): number {
    let distance = c2 - c1 - this.pocket.millRadius;
    if (this.pocket.roughing) {
  ...
  getPocketDistTop() {
    const relevantValue = Math.max(this.pocket.p1.y, this.pocket.p2.y);
    return this.getPointDistance(this.yAxis.absValue, relevantValue);
  }

  getPocketDistBottom() {
    const relevantValue = Math.min(this.pocket.p1.y, this.pocket.p2.y);
    return this.getPointDistance(relevantValue, this.yAxis.absValue);
  }

  getPocketDistLeft() {
    const relevantValue = Math.min(this.pocket.p1.x, this.pocket.p2.x);
    return this.getPointDistance(relevantValue, this.xAxis.absValue);
  }

  getPocketDistRight() {
    const relevantValue = Math.max(this.pocket.p1.x, this.pocket.p2.x);
    return this.getPointDistance(this.xAxis.absValue, relevantValue);
  }

  getPocketDistZ() {
    let relevantValue = Math.max(this.pocket.p1.z, this.pocket.p2.z);
    if (this.pocket.roughing) {
