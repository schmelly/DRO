...
export class ContourViewComponent {
  ...
  getTopLeft(): Point2D {
    const pX = Math.min(this.contour.p1.x, this.contour.p2.x);
    const pY = Math.max(this.contour.p1.y, this.contour.p2.y);
    return { x: pX, y: pY };
  }

  getTopRight(): Point2D {
    const pX = Math.max(this.contour.p1.x, this.contour.p2.x);
    const pY = Math.max(this.contour.p1.y, this.contour.p2.y);
    return { x: pX, y: pY };
  }
  ...
  getBottomLeft(): Point2D {
    const pX = Math.min(this.contour.p1.x, this.contour.p2.x);
    const pY = Math.min(this.contour.p1.y, this.contour.p2.y);
    return { x: pX, y: pY };
  }
  ...
  getMidPoint(): Point2D {
    const mX = (this.contour.p1.x + this.contour.p2.x) / 2;
    const mY = (this.contour.p1.y + this.contour.p2.y) / 2;
    return { x: mX, y: mY };
  }
  ...
  getContourDrillTransform(): string {
    ...
    let xMin = Math.min(this.contour.p1.x, this.contour.p2.x);
    let xMax = Math.max(this.contour.p1.x, this.contour.p2.x);
    let yMin = Math.min(this.contour.p1.y, this.contour.p2.y);
    let yMax = Math.max(this.contour.p1.y, this.contour.p2.y);
    let xLocation;
    let yLocation;
  ...
  getContourDrillTransformZ(): string {
    ...
    let zMax = Math.max(this.contour.p1.z, this.contour.p2.z);
    let zLocation;
  ...
  getPointDistance(c1: number, c2: number): number {
    let distance = c2 - c1 - this.contour.millRadius;
    if (this.contour.roughing) {
